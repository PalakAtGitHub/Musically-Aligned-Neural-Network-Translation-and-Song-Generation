...
class HierarchicalMelodyEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        # CNN extracts local patterns (2-3 note motifs)
        ...
        # BiGRU models how phrases evolve
        ...
        # Self-attention for long-range dependencies (verse vs chorus, repetition)
    ...
    def forward(self, melody_features):
        ...
        # Transpose for Conv1d: [batch, input_dim, num_notes]
        x = melody_features.transpose(1, 2)
        x = F.relu(self.conv1(x))
        x = self.conv_dropout(F.relu(self.conv2(x)))
        # Back to [batch, num_notes, conv_channels]
        x = x.transpose(1, 2)
