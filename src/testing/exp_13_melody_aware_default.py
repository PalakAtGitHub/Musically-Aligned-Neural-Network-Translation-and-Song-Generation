"""
Experiment 13: MelodyAwareDecoderLayer — full 10-epoch training + default
generation evaluation on 198 strict held-out examples.

Architecture: MCNST with MelodyAwareDecoderLayer wrapping all 18 decoder layers.
All original IndicTrans2 decoder layers are frozen.  Trainable: melody_attn
sublayers (47M), lm_head (126M), embed_tokens (126M), melody_encoder (3M),
fusion (4.5M), aligner (328K), loss log_vars (7).  Total ~306M.

Saves:
  - checkpoints/melody_aware_model.pt  (best val_loss checkpoint)
  - logs/exp_13_melody_aware_default_results.json
"""
...
print("=" * 70)
print("EXPERIMENT 13: MelodyAwareDecoderLayer — Training + Default Eval")
print("=" * 70)

# ------------------------------------------------------------------
# 1. Build model and wrap decoder layers
# ------------------------------------------------------------------
print("\n[1/7] Building model and wrapping decoder layers...")
...
# Freeze decoder layer_norm only — keep lm_head + embed_tokens trainable
...
print(f"\n[3/7] Training for {NUM_EPOCHS} epochs...")
print("=" * 70)
...
for epoch in range(1, NUM_EPOCHS + 1):
    ...
    gate_vals = []
    for i in range(num_layers):
        gate_vals.append(model.seq2seq.model.decoder.layers[i].melody_gate.item())

    print(f"  Epoch {epoch}: train_loss={avg_train:.4f}, val_loss={avg_val:.4f}")
    print(f"    gates (min/mean/max): {min(gate_vals):.4f} / "
          f"{sum(gate_vals)/len(gate_vals):.4f} / {max(gate_vals):.4f}")

    history.append({
        'epoch': epoch, 'train_loss': avg_train, 'val_loss': avg_val,
        'gate_values': [round(g, 6) for g in gate_vals],
    })
    ...
    if avg_val < best_val_loss:
        ...
        torch.save({
            ...
            'val_loss': avg_val,
            'architecture': 'melody_aware_decoder',
        }, CHECKPOINT_PATH)
...
with open("checkpoints/melody_aware_training_history.json", 'w') as f:
    json.dump(history, f, indent=2)

# ------------------------------------------------------------------
# 6. Load best checkpoint and evaluate on strict held-out
# ------------------------------------------------------------------
print(f"\n[4/7] Loading best melody-aware checkpoint...")
...
print(f"\n[6/7] Generating on {len(strict_heldout)} examples (default decoding)...")
print("=" * 70)
