"""
Test the trained MCNST model (Two-Stage Architecture, IndicTrans2)
...
def test_on_examples(model, num_examples=5):
    ...
    for i, example in enumerate(data[:num_examples]):
        ...
        with torch.no_grad():
            ...
            two_stage_text = model.postprocess_tgt(two_stage_text)[0]
...
def test_new_song(model, english_line, midi_path, use_constrained=True):
    ...
    # Preprocess English for IndicTrans2
    preprocessed = model.preprocess_src(english_line)
    ...
    with torch.no_grad():
        ...
        if use_constrained:
            ...
            constrained_text = tokenizer.decode(constrained_tokens[0], skip_special_tokens=True)
            constrained_text = model.postprocess_tgt(constrained_text)[0]
